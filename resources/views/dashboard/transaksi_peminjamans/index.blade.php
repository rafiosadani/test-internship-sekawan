                ...
                <div class="card border-primary">
                    <div class="card-header py-3 d-flex justify-content-between align-items-center">
                        ...
                        <div class="card-tools">
                            <a href="/dashboard/transaksi-peminjamans/create" class="btn btn-primary btn-sm float-right"><i class="fas fa-fw fa-plus"></i> Tambah Peminjaman Kendaraan</a>
                        </div>
                    </div>
                            ...
                            <tbody>
                                @if ($peminjamans->count() > 0)
                                    @foreach ($peminjamans as $peminjaman)
                                        <tr>
                                            {{-- <td class="text-center align-middle text-dark">{{ $loop->iteration + ($peminjamans->currentPage() - 1) * $peminjamans->perPage() }}</td> --}}
                                            ...
                                            <td class="align-middle text-center">
                                                <form action="/dashboard/transaksi-peminjamans/destroy/{{ $peminjaman->id_peminjaman }}" method="post" class="d-inline">
                                                    @method('put')
                                                    @csrf
                                                    <button type="submit" class="badge badge-danger border-0" onclick="return confirm('Anda yakin ingin menghapus data peminjaman kendaraan ini?');"><i class="fas fa-fw fa-trash"></i> Hapus</button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                                ...
                                @if ($peminjamans_sewa->count() > 0)
                                    @foreach ($peminjamans_sewa as $peminjaman_sewa)
                                        <tr>
                                            ...
                                            <td class="align-middle text-center">
                                                <form action="/dashboard/transaksi-peminjamans/destroy/{{ $peminjaman_sewa->id_peminjaman }}" method="post" class="d-inline">
                                                    @method('put')
                                                    @csrf
                                                    <button type="submit" class="badge badge-danger border-0" onclick="return confirm('Anda yakin ingin menghapus data peminjaman kendaraan ini?');"><i class="fas fa-fw fa-trash"></i> Hapus</button>
                                                </form>
